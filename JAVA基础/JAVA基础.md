...
#### 四次挥手的time_wait为什么是2MSL，如果超时了会怎么办，第几个包丢了，如果出现大量的time_wait可能出现什么问题，如果硬件资源足够，即带宽等都是够的，还有什么会限制，是不是资源足够的情况下没有其他限制(我认为就是客户端重连会失败，因为TCP连接是4元组)
