...
ArrayList是基于数组的，而一旦添加节点导致容量不够用就会触发扩容函数，ArrayList是基于创建新的数组并将原数据拷贝进新数组数组实现的扩容，需要新开辟一个比当前容量更大一些的数组空间（旧容量的1.5倍），空间复杂度大致为O（n），而复制函数时间复杂度也需要O（n）。（最大数组大小可以达到整型的最大值）  
