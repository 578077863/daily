...
`        Program cur = this;`
`        Object o = super;  报错`
子类字节码中有父类索引，因此可以直接找到父类的各种信息（元信息、方法指令），父类定义的内存全部再子类的内存范围，new开辟的内存空间中，不仅含有子类的数据，也包含父类拿到的数据，子类是父类的延伸，因此子类对象往往比父类对象更占用内存空间。
...
编译期在编译期就可以确认某一个方法的重载版本，但是并不一定能确认它的调用入口。
...
【2】对什么对象执行垃圾回收  
从GC roots开始扫描，最终被判定为不可达的对象，且**经过第一次标记没有被finalize方法复活的对象**  
【3】GC做了什么  
