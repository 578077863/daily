...
1、**Jdk****动态代理**：利用拦截器（必须实现InvocationHandler接口）加上**反射机制**生成一个代理接口的匿名类，在调用具体方法前调用InvokeHandler来处理

2、 **Cglib****动态代理**：利用ASM框架，对代理对象类生成的class文件加载进来，通过**修改其字节码生成子类来进行代理**
...
Http协议在传输的时会携带很多额外信息，如Header数据，而Rpc传输一般可以使用自定义的协议体。所以通常情况下，在相同的核心请求参数条件下，Rpc的数据报文相对于Http的数据报文，Rpc协议传输的数据包体积会小许多。
