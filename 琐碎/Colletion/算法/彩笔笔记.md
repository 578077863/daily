 ...
 */
 /**
 preorder是要计算谁才是 root, inoreder只是确定下左右边界而已
  */
class Solution {
    ...
    private TreeNode rebuild(int[] preorder, int[] inorder, int rootIdx_pre, int leftIdx_in, int rightIdx_In){

        if(leftIdx_in > rightIdx_In) return null;

        //首先根节点得出来
        TreeNode root = new TreeNode(preorder[rootIdx_pre]);

        int rootIdx_in = map.get(root.val);

        root.left = rebuild(preorder, inorder, rootIdx_pre + 1, leftIdx_in, rootIdx_in - 1);

        root.right = rebuild(preorder, inorder, rootIdx_pre + rootIdx_in - leftIdx_in + 1, rootIdx_in + 1, rightIdx_In);
